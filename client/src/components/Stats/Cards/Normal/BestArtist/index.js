...
class BestArtist extends React.Component {
  ...
  refresh = async () => {
    ...
    console.log('Artist', JSON.parse(JSON.stringify(data)));
  ...
  render() {
    ...
    return (
      <div className={s.root} style={{ backgroundImage: `url(${stats[0].artists[0].images[0].url})` }}>
        <IntervalModifier
        ...
        <div className={s.content}>
          <div className={s.text}>
            ...
            <div className={s.title}>{stats[0].artists[0].name}</div>
          </div>
        </div>
      </div>
    )
