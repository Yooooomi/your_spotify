...
export const selectRawIntervalDetail = createSelector(
  ...
  (state): RawIntervalDetail =>
    getRawIntervalDetail(fromReduxIntervalDetail(state.intervalDetail), state.user),
);

export const selectPublicToken = createSelector(selectUserState, (state) => state.publicToken);
export const selectIsPublic = createSelector(selectPublicToken, (token) => Boolean(token));
